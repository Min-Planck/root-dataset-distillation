...
class Forgetting(EarlyTrain):
    ...
    def after_loss(self, outputs, loss, targets, batch_inds, epoch):
        with torch.no_grad():
            ...
            batch_inds_tensor = torch.tensor(batch_inds, device=self.device)
            mask = (self.last_acc[batch_inds_tensor] - cur_acc) > 0.01
            self.forgetting_events[batch_inds_tensor[mask]] += 1.
            self.last_acc[batch_inds_tensor] = cur_acc
    ...
    def select(self, **kwargs):
        ...
        if not self.balance:
            ...
            for c in range(self.num_classes):
                c_indx = self.train_indx[self.targets == c]
