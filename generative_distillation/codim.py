...
def info_nce_loss(features, features_aug, temperature=0.1):
    """
    features: [Batch_size, dim] - Feature ảnh gốc
    features_aug: [Batch_size, dim] - Feature ảnh augment
    """
    # Tính độ tương đồng (logits)
    # Tương đồng giữa feature gốc và tất cả feature aug
    # Ma trận [Batch, Batch]
    similarity_matrix = torch.matmul(features, features_aug.T)

    # Tạo nhãn (Labels)
    # Ảnh thứ i của batch gốc phải khớp với ảnh thứ i của batch aug
    batch_size = features.shape[0]
    labels = torch.arange(batch_size).to(features.device) # [0, 1, 2, ..., N-1]

    # Tính Cross Entropy Loss với temperature
    # Tự động thực hiện phép tính log(exp(pos) / sum(exp(all)))
...
def train(args, epoch, generator, discriminator, optim_g, optim_d, trainloader, criterion, aug, aug_rand, mode='gan'):
    ...
    for batch_idx, (img_real, lab_real) in enumerate(trainloader):
        ...
        img_syn_aug = aug(img_syn)
        _, _, proj_fake = discriminator(img_syn, get_features=True)
        _, _, proj_fake_aug = discriminator(img_syn_aug, get_features=True)
        loss_div = info_nce_loss(proj_fake, proj_fake_aug) 

        gen_class = criterion(gen_class, lab_real)
        
        gen_loss = - gen_source + gen_class + loss_div * args.lambda_div
        ...
        disc_real_source, disc_real_class, proj_real = discriminator(img_real, get_features=True)
        img_real_aug = aug(img_real)
        _, _, proj_real_aug = discriminator(img_real_aug, get_features=True)

        loss_ctr = info_nce_loss(proj_real, proj_real_aug)
        ...
        disc_loss = disc_fake_source - disc_real_source + disc_fake_class + disc_real_class + args.lambda_ctr * loss_ctr + gradient_penalty
        disc_loss.backward()
        optim_d.step()
...
def start_codim(args, trainset, testset):
